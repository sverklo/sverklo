...
const MODEL_DIR = join(homedir(), ".codesearch", "models");
  ...
  if (!modelPath || !tokenizerPath) {
    logError(
      "Model files not found. Falling back to lightweight embeddings.\n" +
        "For better search quality, place model.onnx and tokenizer.json in ~/.codesearch/models/"
    );
    initialized = true;
    return;
  }
...
function findFile(filename: string): string | null {
  // Check ~/.codesearch/models/ first, then bundled
